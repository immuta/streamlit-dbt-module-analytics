...
manifest = streamlit.file_uploader(
    "Upload your manifest file.", type="json", accept_multiple_files=False
)
...
for ii, s in edge_df.iterrows():
    if s["source"] in selected_nodes or s["target"] in selected_nodes:
        G_single.add_edge(
            s["source_product_node_name"],
            s["target_product_node_name"],
            weight=1,
            **s.to_dict()
        )
...
nx.set_node_attributes(
    G_single,
    node_df.set_index("product_node_name")
    .loc[list(G_single.nodes)]
